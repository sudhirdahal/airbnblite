...
const SearchBar = ({ onSearch }) => {
  const [params, setParams] = useState({ location: '', checkInDate: '', checkOutDate: '', guests: '', amenities: [] });
  const [showFilters, setShowFilters] = useState(false);
  const availableAmenities = [{ id: 'WiFi', icon: Wifi }, { id: 'Kitchen', icon: Utensils }, { id: 'Pool', icon: Waves }, { id: 'Parking', icon: Car }, { id: 'TV', icon: Tv }];

  const handleChange = (e) => { setParams({ ...params, [e.target.name]: e.target.value }); };
  const toggleAmenity = (id) => { const newAmenities = params.amenities.includes(id) ? params.amenities.filter(a => a !== id) : [...params.amenities, id]; setParams({ ...params, amenities: newAmenities }); };
  const handleSearch = () => { onSearch({ ...params, amenities: params.amenities.join(',') }); setShowFilters(false); };
  const handleClear = () => { const cleared = { location: '', checkInDate: '', checkOutDate: '', guests: '', amenities: [] }; setParams(cleared); onSearch(cleared); };

  return (
    <div style={containerStyle}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        {/* ADDED CLASS: search-wrapper */}
        <div className="search-wrapper" style={searchWrapperStyle}>
          
          {/* ADDED CLASS: search-input-group */}
          <div className="search-input-group" style={inputGroupStyle}>
            <div style={labelStyle}><MapPin size={14} /> Location</div>
            <input type="text" name="location" placeholder="Where to?" value={params.location} onChange={handleChange} className="search-input" style={inputStyle} />
          </div>

          <div className="search-divider" style={dividerStyle} />
          ...
          <div style={buttonGroupStyle}>
            {(params.location || params.checkInDate || params.guests || params.amenities.length > 0) && (<button onClick={handleClear} style={clearButtonStyle}><X size={18} /></button>)}
            <button onClick={handleSearch} style={searchButtonStyle}><Search size={18} color="white" /> <span style={{ color: 'white', fontWeight: 'bold' }}>Search</span></button>
          </div>
        </div>
          ...
          {showFilters && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={filterPanelStyle}>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                {availableAmenities.map(a => (<button key={a.id} onClick={() => toggleAmenity(a.id)} style={amenityBtnStyle(params.amenities.includes(a.id))}><a.icon size={16} /> {a.id}</button>))}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
