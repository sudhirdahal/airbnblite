...
import { Link } from 'react-router-dom'; // --- NEW: Navigation Link ---
import { motion } from 'framer-motion';
import { Sparkles, Home, User } from 'lucide-react';

/**
 * Hero Component (v3): Interactive Dashboard Banner.
 * Now features clickable user profiles and tactile feedback.
 */
const Hero = ({ user }) => {
  ...
  return (
    <div style={bannerContainerStyle}>
      ...
      >
        <div style={topRowStyle}>
          <div style={iconBadgeStyle}>
            {userRole === 'admin' ? <Home size={16} /> : <Sparkles size={16} />}
          </div>
          {/* Breadcrumb now acts as a Home link */}
      ...
      {user && (
        <Link to="/profile" style={{ textDecoration: 'none', color: 'inherit' }}>
          <motion.div 
            ...
            whileHover={{ scale: 1.02, backgroundColor: '#fcfcfc', borderColor: '#ff385c' }} // Tactile feedback
            whileTap={{ scale: 0.98 }}
      ...
      {/* --- OLD CODE (Preserved as requested) ---
      /* 
      // Non-interactive profile card:
      {user && (
        <div style={profileCardStyle}>
          <div style={avatarCircleStyle}>{user.name.charAt(0)}</div>
          <div>{user.name}</div>
        </div>
      )}
      */}
