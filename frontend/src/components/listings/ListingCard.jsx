...
/**
 * ListingCard Component: Displays summary info for a listing.
 * Now features searchable locations and enhanced animations.
 */
const ListingCard = ({ listing, userRole, onWishlistUpdate, onSearch }) => {
  ...
  useEffect(() => {
    ...
    if (user && user.wishlist) {
      setIsWishlisted(user.wishlist.includes(listing._id));
    }
  }, [listing._id]);
  ...
  const toggleWishlist = async (e) => {
    ...
    try {
      ...
      if (newStatus) toast.success('Saved to Wishlist!', { icon: '❤️' });
      else toast('Removed from Wishlist', { icon: '🗑️' });
  ...
  // --- NEW: Searchable Location Handler ---
  const handleLocationClick = (e) => {
    e.preventDefault();
    e.stopPropagation(); // Prevent navigating to detail page
    
    // If we are on the homepage, trigger search. Otherwise, navigate home with search state.
    if (onSearch) {
      onSearch({ location: listing.location });
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      navigate('/', { state: { searchLocation: listing.location } });
    }
    toast(`Searching for stays in ${listing.location}`, { icon: '🔍' });
  };
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -5 }}
      style={{ position: 'relative', width: '300px', margin: '1rem' }}
    >
      <Link to={`/listing/${listing._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
        <div style={{ borderRadius: '12px', overflow: 'hidden', cursor: 'pointer' }}>
          <div style={{ position: 'relative', width: '100%', aspectRatio: '1 / 1' }}>
            ...
            <button onClick={toggleWishlist} style={wishlistBtnStyle(isWishlisted)}>
              <Heart size={24} fill={isWishlisted ? '#ff385c' : 'rgba(0,0,0,0.3)'} />
            </button>
          </div>
          <div style={{ padding: '0.8rem 0' }}>
            ...
            {/* --- NEW: Interactive Location Text --- */}
            <motion.div 
              onClick={handleLocationClick}
              whileHover={{ color: '#ff385c', x: 2 }}
...
const wishlistBtnStyle = (active) => ({
  position: 'absolute', top: '1rem', right: '1rem', background: 'none', border: 'none', cursor: 'pointer', padding: 0,
  color: active ? '#ff385c' : 'rgba(0,0,0,0.5)', filter: 'drop-shadow(0 0 2px rgba(0,0,0,0.3))'
});
