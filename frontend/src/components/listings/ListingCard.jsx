...
const ListingCard = ({ 
  ...
  return (
    <motion.div 
      ...
      onMouseEnter={() => { if (!isMobile) setIsHovered(true); onHover && onHover(); }}
      onMouseLeave={() => { if (!isMobile) setIsHovered(false); onLeave && onLeave(); }}
      ...
      <Link to={`/listing/${listing._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
        ...
        <div style={contentAreaStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={titleStyle(isMobile)}>{listing.title}</h3>
          ...
          <div style={priceContainerStyle}>
            <span style={priceStyle(isMobile)}>${listing.rate}</span> 
...
const titleStyle = (isMobile) => ({ margin: 0, fontSize: isMobile ? '1rem' : theme.typography.sizes.sm, fontWeight: theme.typography.weights.bold, color: theme.colors.charcoal, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' });
...
const priceStyle = (isMobile) => ({ fontWeight: theme.typography.weights.extraBold, fontSize: isMobile ? '1rem' : theme.typography.sizes.sm, color: theme.colors.charcoal });
