...
/**
 * ============================================================================
 * LISTING DETAIL PAGE (V9 - CINEMATIC RESTORATION)
 * ============================================================================
 */
const ListingDetail = ({ userRole, user, onChatOpened }) => { 
  ...
  const displayImages = listing.images && listing.images.length > 0 ? listing.images : ['https://via.placeholder.com/800'];
  ...
  return (
    <div style={{ position: 'relative', paddingBottom: isMobile ? '100px' : '0' }}>
      ...
      <div style={{ maxWidth: '100%', width: '100%', margin: isMobile ? '0' : '2rem auto', padding: isMobile ? '0' : '0 2rem' }}>
        ...
        {!isMobile && <h1 style={{ marginBottom: '1.5rem', fontSize: '1.8rem' }}>{listing.title}</h1>}
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '0' : '4rem' }}>
          ...
          <div style={{ flex: 2 }}>
            ...
            {/* --- RESTORED: CINEMATIC GRID GALLERY --- */}
            {isMobile ? (
              <img src={displayImages[0]} style={{ width: '100%', height: '300px', objectFit: 'cover' }} onClick={() => openLightbox(0)} />
            ) : (
              <div style={galleryGrid}>
                <div onClick={() => openLightbox(0)} style={mainImageContainer}>
                  <img src={displayImages[0]} style={galleryImg} alt="Featured" />
                </div>
                <div style={sideTilesContainer}>
                  {displayImages.slice(1, 5).map((img, i) => (
                    <div key={i} onClick={() => openLightbox(i + 1)} style={sideTile}>
                      <img src={img} style={galleryImg} alt={`View ${i + 1}`} />
                    </div>
                  ))}
                  <button onClick={() => openLightbox(0)} style={showAllBtn}>
            ...
            <div style={{ padding: isMobile ? '1.5rem' : '2.5rem 0' }}>
              {isMobile && <h1 style={{ fontSize: '1.6rem', marginBottom: '0.5rem' }}>{listing.title}</h1>}
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#222', fontWeight: '600' }}>
              ...
              {/* RESTORED: HOST DESCRIPTION */}
              <div style={{ marginTop: '2.5rem', borderTop: '1px solid #eee', paddingTop: '2.5rem' }}>
                <h2 style={{ fontSize: '1.4rem' }}>Hosted by {listing.host?.name || 'Professional Host'}</h2>
      ...
      {isMobile && (
        <div style={mobileStickyBar}>
          <div>
            <div style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>${listing.rate} <span style={{ fontWeight: 'normal', fontSize: '0.9rem' }}>night</span></div>
            <div style={{ fontSize: '0.8rem', textDecoration: 'underline', color: '#222' }}>{pricing.nights > 0 ? `${pricing.nights} nights` : 'Select dates'}</div>
          </div>
...
const galleryImg = { width: '100%', height: '100%', objectFit: 'cover', transition: 'filter 0.3s ease' };
const showAllBtn = { position: 'absolute', bottom: '20px', right: '20px', backgroundColor: '#fff', border: '1px solid #222', borderRadius: '8px', padding: '0.5rem 1rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.85rem', boxShadow: '0 2px 4px rgba(0,0,0,0.1)' };
