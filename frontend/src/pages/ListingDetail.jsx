...
import { formatDistanceToNow } from 'date-fns'; // --- NEW: High-Fidelity Time ---
 ...
 * ============================================================================
 * This page represents the 'Frontend Brain' of the reservation system. 
 * It has evolved from a static image viewer to a dynamic pricing engine 
 * with real-time occupancy validation and integrated chat presence.
 */
const ListingDetail = ({ userRole, user, onChatOpened }) => { 
  ...
  useEffect(() => {
    if (dateRange[0] && dateRange[1] && listing) {
      ...
      if (diffNights > 0) {
        /* --- HISTORICAL STAGE 1: FLAT PRICING (FLAWED) ---
         * Initially, the total was just Nights * Rate. 
         * It didn't account for extra guests or service fees.
         * const total = diffNights * listing.rate;
         */

        // CURRENT LOGIC (Phase 5+): Occupancy-Aware Multi-Guest Pricing
  ...
  const fetchListingAndReviews = async () => {
    try {
      ...
      setListing(listingRes.data); setReviews(reviewsRes.data); 
      setTakenDates(takenRes.data); setChatHistory(chatRes.data);
    ...
    <div style={{ position: 'relative' }}>
      <div style={{ maxWidth: '100%', width: '100%', margin: '2rem auto', padding: '0 2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '4rem', marginTop: '2.5rem' }}>
          <div style={{ flex: 2 }}>
            <img src={listing.images[0]} style={{ width: '100%', height: '520px', objectFit: 'cover', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }} />
            ...
            <div style={{ marginTop: '4rem', borderTop: '1px solid #eee', paddingTop: '3rem' }}>
              <div style={{ display: 'flex', gap: '4rem', alignItems: 'flex-start', marginBottom: '3rem' }}>
                <div>
                  <h2 style={{ fontSize: '2.2rem', margin: 0, fontWeight: '800' }}><Star size={28} fill="#000" /> {listing.rating}</h2>
                  <p style={{ color: '#717171', fontWeight: '600' }}>{listing.reviewsCount} reviews</p>
                </div>
                ...
                {reviews.map(r => (
                  <div key={r._id} style={{ marginBottom: '1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.8rem' }}>
                      <div style={avatarCircleStyle}>{r.userId.name.charAt(0)}</div>
                      <div>
                        <div style={{ fontWeight: 'bold' }}>{r.userId.name}</div>
                        {/* --- FIXED: Professional Time-Ago --- */}
          ...
          <div style={{ flex: 1 }}>
            <div style={sidebarCardStyle}>
              ...
              <button onClick={() => navigate('/pay')} style={reserveBtnStyle}>Reserve Now</button>
            </div>
          </div>
        </div>
      </div>
