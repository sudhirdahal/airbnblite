...
import { Map as MapIcon, List, ArrowUpDown } from 'lucide-react'; // --- NEW: Icon ---
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 4rem', maxWidth: '2560px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', border: '1px solid #ddd', padding: '0.6rem 1rem', borderRadius: '12px', backgroundColor: '#fff' }}>
          ...
          <select 
            ...
            style={{ border: 'none', outline: 'none', fontSize: '0.9rem', fontWeight: '600', cursor: 'pointer', backgroundColor: 'transparent' }}
          >
...
const App = () => {
  ...
  const [sort, setSort] = useState('newest'); // --- NEW: Sort State ---
  ...
  const fetchListings = async (params = searchParams, category = activeCategory, currentSort = sort) => {
    setLoading(true);
    try {
      let url = `/listings?sort=${currentSort}&`; // --- NEW: Include Sort ---
    ...
    } catch (err) { console.error(err); } finally { setTimeout(() => setLoading(false), 600); }
  };
  ...
  const handleSortChange = (newSort) => { setSort(newSort); fetchListings(searchParams, activeCategory, newSort); }; // --- NEW ---
