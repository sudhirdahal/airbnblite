const mongoose = require('mongoose');
const dotenv = require('dotenv');
const User = require('./models/User');
const Listing = require('./models/Listing');
const bcrypt = require('bcryptjs');

dotenv.config();

const seedData = async () => {
  try {
    await mongoose.connect(process.env.MONGO_URI);
    console.log('Connected to MongoDB for seeding...');

    await User.deleteMany();
    await Listing.deleteMany();

    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash('admin123', salt);
    const admin = await User.create({
      name: 'Admin User',
      email: 'admin@example.com',
      password: hashedPassword,
      role: 'admin',
      isVerified: true
    });

    const listings = [
      {
        title: 'Cozy Cottage in the Woods',
        location: 'Aspen, Colorado',
        coordinates: { lat: 39.1911, lng: -106.8175 },
        category: 'cabins',
        description: 'A peaceful retreat surrounded by nature.',
        fullDescription: 'Experience the ultimate mountain getaway in this charming cedar cottage...',
        rate: 150,
        rating: 4.9,
        reviewsCount: 128,
        images: [
          'https://images.unsplash.com/photo-1470770841072-f978cf4d019e?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1449156001437-3a1f93977c71?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=1200&q=80'
        ],
        host: { name: 'Alex Johnson', avatar: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=150&q=80' },
        amenities: ['Fireplace', 'WiFi', 'Kitchen'],
        adminId: admin._id
      },
      {
        title: 'Modern Apartment in the City',
        location: 'New York, New York',
        coordinates: { lat: 40.7128, lng: -74.0060 },
        category: 'iconic',
        description: 'Stylish and convenient living in the heart of downtown.',
        fullDescription: 'This sleek loft is located in the vibrant heart of Manhattan...',
        rate: 220,
        rating: 4.8,
        reviewsCount: 95,
        images: [
          'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1493809842364-78817add7ffb?auto=format&fit=crop&w=1200&q=80'
        ],
        host: { name: 'Sarah Lee', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=150&q=80' },
        amenities: ['City View', 'Gym Access'],
        adminId: admin._id
      },
      {
        title: 'Luxury Villa by the Beach',
        location: 'Malibu, California',
        coordinates: { lat: 34.0259, lng: -118.7798 },
        category: 'pools',
        description: 'Relax and enjoy the sound of the ocean.',
        fullDescription: 'Paradise awaits at this stunning beachfront villa...',
        rate: 550,
        rating: 5.0,
        reviewsCount: 42,
        images: [
          'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1499793983690-e29da59ef1c2?auto=format&fit=crop&w=1200&q=80'
        ],
        host: { name: 'Michael Chen', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80' },
        amenities: ['Infinity Pool', 'Beach Access'],
        adminId: admin._id
      },
      {
        title: 'Historic Townhouse with Garden',
        location: 'London, United Kingdom',
        coordinates: { lat: 51.5074, lng: -0.1278 },
        category: 'breakfast',
        description: 'Elegant living in a quiet, historic neighborhood.',
        fullDescription: 'Discover the charm of Old London in this Victorian townhouse...',
        rate: 280,
        rating: 4.7,
        reviewsCount: 67,
        images: [
          'https://images.unsplash.com/photo-1583608205776-bfd35f0d9f83?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?auto=format&fit=crop&w=1200&q=80'
        ],
        host: { name: 'James Smith', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80' },
        amenities: ['Private Garden', 'Heating'],
        adminId: admin._id
      },
      {
        title: 'Desert Oasis with Stargazing Deck',
        location: 'Joshua Tree, California',
        coordinates: { lat: 34.1333, lng: -116.3131 },
        category: 'arctic',
        description: 'Unique architectural home in the high desert.',
        fullDescription: 'Immerse yourself in the magic of Joshua Tree...',
        rate: 320,
        rating: 4.9,
        reviewsCount: 84,
        images: [
          'https://images.unsplash.com/photo-1505576391880-b3f9d713dc4f?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1518732714860-b62714ce0c59?auto=format&fit=crop&w=1200&q=80'
        ],
        host: { name: 'Luna Ray', avatar: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=150&q=80' },
        amenities: ['Stargazing Deck', 'Fire Pit'],
        adminId: admin._id
      },
      {
        title: 'Tropical Treehouse Retreat',
        location: 'Bali, Indonesia',
        coordinates: { lat: -8.4095, lng: 115.1889 },
        category: 'beach',
        description: 'Sleep among the palm trees in a bamboo paradise.',
        fullDescription: 'Live your jungle dreams in this stunning eco-friendly bamboo treehouse...',
        rate: 190,
        rating: 4.8,
        reviewsCount: 156,
        images: [
          'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?auto=format&fit=crop&w=1200&q=80',
          'https://images.unsplash.com/photo-1544644181-1484b3fdfc62?auto=format&fit=crop&w=1200&q=80'
        ],
        host: { name: 'Wayan Bali', avatar: 'https://images.unsplash.com/photo-1531427186611-ecfd6d936c79?auto=format&fit=crop&w=150&q=80' },
        amenities: ['Jungle View', 'Rain Shower'],
        adminId: admin._id
      }
    ];

    await Listing.insertMany(listings);
    console.log('Successfully seeded listings with categories!');
    mongoose.connection.close();
  } catch (err) {
    console.error('Error seeding data:', err);
    process.exit(1);
  }
};

seedData();
